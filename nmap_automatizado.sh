...
if [ $(id -u) -ne 0 ]; then
	echo -e "\n[!] Debes ser usuario administrador para ejecutar el script -> (sudo $0)"
exit 1
fi
...
if [ "$(echo $?)" == "0" ]; then
  ...
  read -p "Introduce la IP: " ip
    ...
    *)
	...
	echo -e "\n[!] Hay que instalar las dependencias" && apt update >/dev/null && apt install nmap -y >/dev/null && echo -e "\nDependencias instaladas"
fi
